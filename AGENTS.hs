...
spliceSites :: [String]
-- ^ `writeOne' is two writes where a row sprouts — the blob, then the cut — and
-- both leave through the same door.
...
cmdNotes =
  ...
  , Note "THE ID IS THE STORE'S TO HAND OUT and the STAMP IS THE ENTRY'S OWN (`stampedEntry'): a drawer claiming an ORG_GLANCE_ID has that line CUT and the minted id written in its place, the blob's path, the answer and the row's identity naming ONE id; an ORG_GLANCE_CREATION_TIME the drawer already spells is left alone, a second line of one key making the value depend on which of the two a reader takes and a row moved out of the inbox carrying the stamp its jot was written with." [Test]
  ...
  , Note "A TAG RUN GAINED BY AN INBOX ROW NAMES ITS DESTINATION, and the sprout is a rule of the INBOX WRITE rather than of one verb: `planCommand' decides it once a row's edits are known, so `add-tag', a `set-title' spelling a run and a `rename-tag' all inherit it and no command carries a flag for it. A TOP-LEVEL, id-less row of the store's own inbox whose EDITED title line wears a run leaves: the subtree is composed as a blob by `blobDocument' — the run's first tag joined by `addTagEditsIn', which folds what the run already wears, under a minted id and the request's ONE clock stamp — and written under the EMPTY CREATE PIN, and only then is it spliced out of `inbox.org'. BLOB FIRST, CUT SECOND: a failure between the two leaves a DUPLICATE and never a loss, and the answer says so, naming the inbox and the id already written. The cut rides the inbox's ONE per-file write beside whatever the rows staying there edit, so the batch rule is untouched and several inbox rows move in one cut. THE CLIENT'S PIN IS THE PLAN'S OWN WALL (`stalePin'), read before the plan mints anything, so a stale tab draws no uuid; the inbox is read ONCE and only where a candidate stands, an ordinary tag write opening no file at all." [Test]
  , Note "A MOVED ROW ANSWERS UNDER THE ID IT ARRIVES AT, with `from' naming the id the request spelled: the sheet's point follows the row rather than the name it had, every log line and refusal names `from', and a COUNT IS OF ROWS AND NEVER OF IDS, one row answering under two. WHAT DOES NOT MOVE: a row already carrying an ORG_GLANCE_ID, a tree with no store to mint into, an edit leaving NO run — `remove-tag' of the last tag does not move it back — and every row outside the inbox; each keeps the in-place edit. A CHILD IS NO ROW OF ITS OWN (`recordsOf' keeps top entries), so nothing names one: it rides the subtree its parent takes." [Test]
