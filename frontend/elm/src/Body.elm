...
blank : Row
blank = Row "" Para Element Nothing Nothing 0 0 "" "" [] 0 1 False
...
propId : Int -> String
propId n = drawerId ++ String.fromInt n
...
-}
draftPairId : String
draftPairId = drawerId ++ "D"

propIndex : String -> Maybe Int
propIndex id = String.toInt (String.dropLeft (String.length drawerId) id)
...
rowsFrom lines own headCells kids =
    let
        head = { blank | id = "H", kind = Head, grain = Element, cells = headCells }
        ...
        rowsIn prefix owner lvl a b =
            let
                blocks = Scan.blocksInRange arr a b
                idAt k = prefix ++ "B" ++ String.fromInt k
            in
            List.indexedMap
                (\i b_ ->
                    let
                        held = cut lines b_.from b_.to
                    in
                    { blank
                        | id = idAt i
                        , kind = Para
                    ...
                    let
                        stop =
                            case rest of
                                next :: _ -> next.line
                                [] -> List.length lines
                        above = List.filter (\( l, _ ) -> l < k.level) seen
                        cid = "C" ++ String.fromInt k.index
                        row =
                            { blank
                                | id = cid
                                , kind = Child
...
setPlanning ( key, value ) plan =
    if value == "" then
        ...
        List.map
            (\p -> if Tuple.first p == key then ( key, value ) else p)
            plan
    else
        plan ++ [ ( key, value ) ]
...
propertyText ( key, value ) = ":" ++ key ++ ": " ++ value
...
readPlanning keywords line =
    let
        ...
        go seen =
            case seen of
                [] -> []
                mark :: rest -> slice mark rest :: go rest
    in
...
bodyText m gone =
    let
        moved r = r.kind == Para && (List.member r.id gone || r.text /= r.was)
        spoken = List.map .id (List.filter moved m.rows)
        ...
        paras = List.reverse (List.filter (\r -> r.kind == Para && not (silenced r)) m.rows)
        splice r out =
            if List.member r.id gone then
                ...
                let
                    ...
                    cut = ownEnd m.rows r
                    shift = indentIn r.text - indentIn r.was
                    under = List.map (nudge shift) (List.take (r.to - cut) (List.drop cut out))
                in
...
markerFor lines reg =
    let
        indent = indentOf (at reg.from lines)
    in
    case reg.kind of
        Plain -> ""
        Item -> itemMarker lines reg.from
        Table -> indent ++ tableRow lines reg.from reg.to
        Block -> indent
        Drawer -> indent
...
nextBullet line o =
    let
        digits = String.fromList (takeWhileList Char.isDigit (String.toList o.bullet))
    in
...
isRule : String -> Bool
isRule line = String.startsWith "|-" (String.trimLeft line)

tableCells : String -> List String
tableCells line =
    let
        cells = List.drop 1 (String.split "|" (String.trim line))
        n = List.length cells
    in
...
caretIn marker =
    let
        open = String.length (indentOf marker) + 1
        firstCell = open + String.length (Maybe.withDefault "" (List.head (tableCells marker)))
    in
...
widest new acc =
    case ( new, acc ) of
        ( [], rest ) -> rest
        ( rest, [] ) -> rest
        ( a :: more, b :: rest ) -> max a b :: widest more rest
...
joinWord m id caret = Maybe.map .word (joinAt m id caret)

pastWord : Row -> String
pastWord top =
    case top.name of
        Just name -> "after the " ++ name
        Nothing ->
...
regionWord kind =
    case kind of
        Item -> "an item at this level"
        Table -> "a row in this table"
        Block -> "a line in this block"
        ...
        Just r ->
            case r.kind of
                ...
                Para ->
                    Just
                        (case caret of
                            Just off -> inside m r (caretLine r off)
                            Nothing -> sibling m r
                        )
...
inside m r line =
    let
        top = outermost m r
        lines = Array.fromList m.lines
        reg = regionAt lines top.from top.to line
    in
...
caretLine r off = r.from + clamp 0 (max 0 (r.to - r.from - 1)) off

holding : { a | rows : List Row } -> Row -> Int -> Row
holding m r line =
    List.foldl
        (\s best -> if s.from >= best.from then s else best)
        r
        (List.filter
            (\s ->
                (s.kind == Para)
...
drafted m id caret = Maybe.map (\j -> joined m j.under (draftRow j j.marker)) (joinAt m id caret)
...
insertion :
    ...
    -> String
    -> Maybe (List Row)
insertion m id caret text = Maybe.map (\j -> joined m j.under (draftRow j text)) (joinAt m id caret)
...
joined m under row =
    let
        kept = List.filter (\r -> r.id /= draftId) m.rows
        owned r = r.from < row.from && List.member under (ownersOf { rows = kept } r.id)
        place out rows =
            case rows of
                [] -> out
                r :: rest ->
...
placeAtLine m line =
    let
        stops = List.filter (\( _, r ) -> r.kind == Para) (List.indexedMap Tuple.pair m.rows)
        -- FOLDED FORWARD, so a nested item wins over the run that holds it.
        covering =
            List.foldl
                (\( i, r ) held -> if r.from <= line && line < r.to then Just i else held)
                Nothing
                stops
        after = List.head (List.filter (\( _, r ) -> r.from >= line) stops)
        last = List.head (List.reverse stops)
    in
    case ( covering, after ) of
        ( Just i, _ ) -> i
        ( Nothing, Just ( i, _ ) ) -> i
        ( Nothing, Nothing ) -> Maybe.withDefault m.at (Maybe.map Tuple.first last)

undrafted : { a | rows : List Row } -> List Row
undrafted m = List.filter (\r -> r.id /= draftId) m.rows

-- CURSOR AND GRAIN

rowById : { a | rows : List Row } -> String -> Maybe Row
rowById m id = List.head (List.filter (\r -> r.id == id) m.rows)

rowAt : { a | rows : List Row, at : Int } -> Maybe Row
rowAt m = nth m.at m.rows

placeOf : { a | rows : List Row, at : Int } -> String -> Int
placeOf m id = Maybe.withDefault m.at (Scan.indexWhere (\r -> r.id == id) m.rows)

shown : Row -> List Cell
shown r = List.filter (\c -> c.val /= "") r.cells

{-| A head never carries an owner, so ownership alone counts a row's kids.
-}
kidsOf : { a | rows : List Row } -> String -> Int
kidsOf m id = List.length (List.filter (\r -> r.owner == Just id) m.rows)
