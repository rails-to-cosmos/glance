...
async function main() {
  ...
  const { broke, bin } = guardEnv("browser-check", BREAKS);
  ...
  // The boot, every re-seed and the git fixture go through this one door.
  const serve = async (dir, usePort) => {
    const { child, base, boot } =
      await serveDaemon(bin, dir, usePort, (said) => { daemonSaid += said; });
    if (!boot.rows || !boot.rows.length)
