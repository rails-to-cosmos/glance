...
|---|---|
| `ref:ID` | rows whose subtree points AT `ID` — shipped today (`docs/query.md`:71-76) |
| `from:ID` | the reverse: rows `ID` points at |
| `ref:ID?kind=SLUG` | the kind test, rhyming with the edge's own file spelling `glance:ID?kind=SLUG` |
| `linked` / `linking` | the existence metas |
...
([`../partial/2026-08-12-relations.md`](../partial/2026-08-12-relations.md):68-89;
`AGENTS.hs`:2916-2931). The edges are the peer's; the rows are glance's; a
...
|---|---|---|
| `ref+:ID` | `splitKey` breaks at the FIRST `:` or `=` (`Filter.hs`:214-218), so the key is `ref+`; `fieldOf` (`Filter.hs`:394) does not know it, and the resolver's fallthrough makes the whole token FREE TEXT (`Filter.hs`:172-182). **Live ground** — a needle searched in `hrSearch` today. | refused |
| `ref:ID*N` | **Dead ground.** `metaOf` needs BOTH ends starred (`Filter.hs`:200-202), so `ID*3` is no meta; it reaches `feRef` unresolved and the atom is `const False` (`Filter.hs`:512-515). | refused |
| `ref:ID..N` | **Dead ground on this axis.** `..` is `rangeMark` (`Filter.hs`:242-245), but `stampOf` is reached only under `stamped key` (`Filter.hs`:419-420, :479-481), and `ref` is not a stamped key; the value passes whole to `feRef` and fails to resolve. | **PICKED** |
...
`refTargetOf`'s own title branch (`Query.hs`:606). A `?q=` is also typed at a
...
`?` first, the anchor behind it (`Query.hs`:599-605).

Two shapes the house has floated before, recorded so they are not re-floated:
`ref:ID/2` ([`../partial/2026-08-12-relations.md`](../partial/2026-08-12-relations.md):340-341)
— dead ground too, and `/` is the shell's own filter door plus a path
separator, and a fallback row id IS a path (`Query.hs`:1100-1106); and
`ref:ID<=3`, which reads well beside the comparison family but puts a
value-OPENING operator (`operatorIn`, `Filter.hs`:272-274) in an infix position,
...
1. **The `?` cut is the sibling's** — `refTargetOf`'s rule, at the first `?`
   (`Query.hs`:599-605), the kind slugged by `kindSlug` (`Query.hs`:645-646).
...
`FILE#K` (`Query.hs`:1100-1106). A fallback id always ends `#` and digits, so
...
(`Filter.hs`:142-145), so `ref:"ID..3"` reads identically to `ref:ID..3`. That
...
- **Required.** The bare `ref:ID` stays exactly one hop, so the closure form is
  ...
  unknown-id law (`Filter.hs`:513-515, `docs/query.md`:74), and no new rule.
...
- **The number.** Recommend **8**, pending one census pass for the longest chain
  ...
  answer it (`../partial/2026-08-12-relations.md`:33-47). The cap has to sit
...
`blocked-by` are one kind (`AGENTS.hs`:2925-2931).
...
(`Filter.hs`:387-390) and this rides that. Today the compile-time half of `ref:`
...
- **Forward (`from:`)** — a row's own `hrLinks`, each `Ref` resolved to a row.
  Resolution is the reverse of `refSpellings` (`Query.hs`:631-632) across three
  namespaces: `hrId`, the title, and `idPropertyOf` for `ViaOrgId`. Note that
  `feRef` is a LINEAR `find` today (`Filter.hs`:377), so the walk builds one
  name→id map per request instead of calling it per edge.
- **Backward (`ref:`)** — target → sources, which is the in-edge index the
  relations proposal already specified (`../partial/2026-08-12-relations.md`:314-329).
...
- **The corpus says the worst case is a different corpus.** Over `~/sync`:
  ...
  nothing (`../partial/2026-08-12-relations.md`:33-47). The reached set is small
  because the graph is sparse.
- **Without the index**, the backward walk costs at most N times what `ref:`
  costs — measured at **1.42 ms** (`../partial/2026-08-12-relations.md`:316), so
...
- **With the index** the same call is 2.39 µs, a 595× at 40 ms of build and
  2.3 MB (`../partial/2026-08-12-relations.md`:316-317). It is a phase-4
...
- **Cycles are real.** The census makes no acyclicity claim, and a mutual pair
  ...
  that (`../partial/2026-08-12-relations.md`:363-364). The visited set is the
  whole of the answer.
...
| `ref:ID..3&kind=x` | `&` is a token separator (`Filter.hs`:133-134) → two tokens, the second free text |

## Implementation sketch

- **`Query.hs`** — the pure walk beside the reference machinery it reads
  (`:565-646`): `closureIn :: Text -> (Text, Int, Maybe Text)` for the two cuts,
  ...
  walk (`../partial/2026-08-12-relations.md`:331-341).
- **`Filter.hs`:368-378** — `FilterEnv` gains the two adjacencies it needs,
  ...
  (`AGENTS.hs`:2911-2915).
- **`Filter.hs`:512-525** — `keyTest … Ref` cuts the value ahead of `feRef`.
  Depth 1 with no kind takes today's branch byte for byte, which is
  conservativity made structural.
- **`Filter.hs`:479-489** — untouched. `ref` is not a `stamped` key, so
...
| **4** | `stInEdges`; `feIn` becomes a lookup | relations stage 4 |
...
- **Path queries (`path:A..B`) — out of scope**, and left open. A path is an
  ...
  ([`../partial/2026-08-12-relations.md`](../partial/2026-08-12-relations.md):339-341).
