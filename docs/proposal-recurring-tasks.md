...
encoder, which is where escaping has to happen):
...
and ONE `fdWriteBuf`.  A `Handle` in `AppendMode` remembers the offset
it opened at, so two writers overwrite each other's lines and the file
ends up shorter than the number of writes — and a marked set across
three files IS three threads, so this is reachable rather than
theoretical.  Every IO failure is swallowed for `noteExternalWrite`'s
reason: the org file is already renamed into place, and failing the
answer over a hint reports a write that landed as one that did not.
...
repeat, the entry being closed.  ONE `replaceSpans` call carrying
composed edit sets:

1. each repeating planning timestamp → its shifted text, which is
...
write, ONE digest, ONE inotify event, ONE frame; two commands would be
two digests, the second drifting on the first.  The ledger line rides
...
whether the row comes back.  WIRE COST: three to five bytes plus the key
on a repeating row, nothing on the rest; repeating rows are a small
minority of this corpus, so a full table pays a few hundred bytes.
...
a third non-column key is renderer-decidable only once `repeats` joins
`hrSearch`, and the parity contract holds the two matchers term for
term.  `-planned:*empty*` already reaches every repeating row, a
repeater riding a planning timestamp by definition.  The key earns its
place when a reader asks for repeating rows specifically, and lands as
one edit to `viewKeys` and `VIEW_KEYS` on both sides.
...
vocabulary over one cell.  `repeats` is where a renderer draws it,
beside the scheduled cell — the cookie belongs to that timestamp.
...
1. **Per-store or per-root.**  PER-STORE: the served root's
   ...
   and a tree holding several stores gets a ledger per store the way
   `externalPathOf` gives it a note file per store.  Per-root needs a
...
6. **The repeater type names.**  Rename `Restart`, `CatchUp` and
   ...
   feature every reader of that code does date math under a word meaning
   its opposite.
