...
-- | The server's own clock.  ONE CLOCK READ PER REQUEST and ONE SPELLING OF IT,
-- taken before any row: the day every date is worked out against and the stamp a
-- minted entry wears come off this one reading, so a request spanning midnight
-- cannot mean two (docs\/invariants.md).
now :: IO ZonedTime
now = getZonedTime

-- | The day AT stands in.
dayAt :: ZonedTime -> Day
