...
export async function chromium(binary = "/usr/bin/chromium") {
  ...
  return {
    ...
    eval: evaluate,
    keys,
    /** THE VIEWPORT, RESIZED.  One addition over the copy this file was taken
     * from (`spikes/2026-09-12-date-cell/cdp.mjs'): an overlay's whole question
     * is what it does when the window runs out, so the rig has to be able to
     * take the window away.  `--window-size' is fixed at launch; device metrics
     * are not. */
    ...
    /** CLIP is the one addition over the copy this file was taken from
     *  (`spikes/2026-09-12-date-overlay/cdp.mjs'): `{x, y, width, height,
     *  scale}' in CSS pixels, so a shot can be CROPPED AND MAGNIFIED on one
     *  cell.  A dress argued about at 1366px wide is a dress nobody can see. */
    ...
    /** The theme the page is asked to believe in, so both are shot in one run
     *  rather than one of them being argued from the other. */
    async theme(which) {
      await cdp.send("Emulation.setEmulatedMedia",
        { features: [{ name: "prefers-color-scheme", value: which }] });
      await sleep(40);
    },
    async close() {
      cdp.close(); proc.kill();
