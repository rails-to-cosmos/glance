/* THE DATE PARSER, PORTED WHOLE from the glue — `readsDate' (15-dates.js:223),
   `englishDate' (`:171'), `shippedDate' (`:112'), `dateWriting' (`:260'),
   `dateGhost' (`:313'), `dateStep' / `dateStepped' (`:325') and everything
   those stand on.  THE BODIES ARE VERBATIM, dedented and no more: what this rig
   judges is WHERE THE GHOST GOES, and a rig that reimplemented the grammar
   would be judging its own reimplementation.

   IT IS ITS OWN FILE BECAUSE THAT IS THE SEAM.  The table owes a date in a cell
   and the sheet owes one on a planning line; two readers over one grammar is
   the drift the drift-pin exists to stop (`test/fixtures/english-dates.json').

   ONE ADDITION over the 2026-09-12-date-cell copy this file was taken from:
   `dateOffers' (`20-sheet.js:916'), which that spike did not need and this one
   cannot be without — the offers are what `TAB' and the arrows act on.  It is
   transcribed at the foot, verbatim.
   The port was taken from `20-sheet.js' and the block has since moved into
   `frontend/glue/15-dates.js', which is the shape that has no second copy in
   it. */
const DATES = (function () {
  ...
  // ============================================ THE OFFERS, TRANSCRIBED VERBATIM
  /* `dateOffers' has NOT moved into `15-dates.js' with the rest of the grammar —
     it is still `frontend/glue/20-sheet.js:916', beside the pair box's own
     `offersFor'.  It is carried here because THE OFFERS ARE HALF THE WIDGET:
     they are what `TAB' and the arrows act on, and a rig that drew a menu of its
     own invention would be measuring its own invention.  The body is the shipped
     body, dedented and no more; `leadTyped' comes with it (`00-core.js:91') and
     `OFFERS' / `NEW_HINT' from `20-sheet.js:840' and `00-core.js:84'. */
  const OFFERS = 6;            // the cap, spelled once (`20-sheet.js:840')
  const NEW_HINT = "new";      // `00-core.js:84'
  /** Does the line the reader TYPED stand as an offer of its own?  An open
   * vocabulary always offers it, except where it FOLDS TO one already listed;
   * an empty field offers no literal (`leadTyped', 00-core.js:91). */
  ...
  return { readsDate, readsStamp, dateGhost, dateWriting, dateNow, dateStep,
           dateStepped, addDays, isoDay, stampOf, dowOf, STAMP,
           dateOffers, leadTyped, OFFERS, NEW_HINT, DATE_VOCAB, MONTH_FULL };
})();
if (typeof module === "object") module.exports = DATES;
